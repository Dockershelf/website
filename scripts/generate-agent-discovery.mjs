...
/**
 * Build-time discovery artifacts from site identity + FEATURE flags.
 * AE3: after flipping FEATURE_BLOG / FEATURE_CONTACT, regenerate (prebuild)
 * and redeploy — CDN TTL may briefly serve stale discovery otherwise.
 */
...
const SKILL_DEFINITIONS = [
  {
    name: "site-discovery",
    description: `Discover canonical ${SITE_NAME} surfaces — llms.txt, sitemap, API catalog, Agent Skills, and MCP.`,
  ...
  {
    name: "citation",
    description: `Citation and attribution rules for ${SITE_NAME} content in AI answers.`,
...
function buildMcpServerCard(version) {
            ...
            inputSchema: {
              ...
              properties: {
                limit: {
                  type: "number",
                  description:
                    "Number of posts to return (default: 5, max: 20)",
                  minimum: 1,
                  maximum: 20,
                },
              },
  ...
  return {
    ...
    serverInfo: {
      name: SITE_NAME,
      title: `${SITE_NAME} — Read-only Agent API`,
      version,
    },
    description: FEATURE_BLOG
      ? `Read-only discovery and blog search for ${SITE_NAME}. No authenticated or transactional tools.`
      : `Read-only discovery for ${SITE_NAME}. No authenticated or transactional tools.`,
